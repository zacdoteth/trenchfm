...
const NAMES = ["degen_ape","sol_maxi","ct_wizard","ngmi_king","gm_queen","wen_moon","bag_holder","alpha_leak","pump_it","ser_rugged","touch_grass","floor_sweep","jeet_proof","diamond_hand","paper_hand","copium_max","hopium_lord","fomo_buyer","whale_alert","smol_brain","big_brain","rug_survivor","moon_soon","few_understand","wagmi_crew","chad_caller","based_dev","anon_whale","memecoin_maxi","onchain_sleuth"];
...
function generateChart(positive) {
  const points = [];
  ...
  // If positive, ensure end > start; if negative, end < start
  if (positive && points[47] < points[0]) {
    const boost = points[0] - points[47] + 10;
    for (let i = 24; i < 48; i++) points[i] += boost * ((i - 24) / 24);
  }
  if (!positive && points[47] > points[0]) {
    const drop = points[47] - points[0] + 10;
    for (let i = 24; i < 48; i++) points[i] -= drop * ((i - 24) / 24);
  }
  return points;
}
  ...
  for (const ring of rings) {
    ...
    for (let j = 0; j < ring.count; j++) {
      ...
      chars.push({id:i,x,z,homeX:x,homeZ:z,hue:r()*360,bobSpeed:1.5+r()*1.5,
        ...
        name:NAMES[Math.floor(r()*NAMES.length)],visible:true});
    }
  }
  return chars;
}
...
export default function PodiumTeleport() {
  ...
  const stateRef = useRef({
    ...
    cameraAngle: 0, cameraHeight: 5, cameraDist: 25, autoRotate: true, spinVelocity: 0, clock: 0,
    // Jumbotron debug params (live-tunable via GUI)
    jmboY: 12, jmboR: 10, jmboTiltY: 14, jmboScale: 0.9,
  });
  ...
  const [announcement, setAnnouncement] = useState(null); // { name, coin, thesis, followers }
  const triggerAnnouncement = useCallback((data) => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      setSpeakerInfo({ name: "degen_ape", coin: firstCoin, idx, thesis: "Bonk just flipped Myro. Dev shipping daily. Binance listing rumored. Easy 3x from here.", txStats, followers: "127K" });
      setCardExpanded(false);
      setChartData(generateChart(firstCoin.positive));
      setVotes({ up: 24, down: 3 });
      triggerAnnouncement({ name: "degen_ape", coin: firstCoin, thesis: "Bonk just flipped Myro. Dev shipping daily. Binance listing rumored. Easy 3x from here.", followers: "127K" });
      setChatMessages(p => [...p, { user: "trench.fm", msg: "⚡ degen_ape is teleporting to the stage..." }]);
    }, 2000);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      setCardExpanded(false);
      setChartData(generateChart(next.coin.positive));
      setVotes({ up: Math.floor(Math.random() * 20) + 5, down: Math.floor(Math.random() * 5) });
      setUserVoted(null);
      // No announcement for queue rotations — just a smooth transition
      setChatMessages(p => [...p, { user: "trench.fm", msg: `⚡ ${next.name || c.name} is teleporting to the stage...` }]);
  ...
  useEffect(() => {
    ...
    function init() {
      ...
      // ═══ JUMBOTRON — LED screen behind stage (MAGICAL BILLBOARD — always faces camera) ═══
      // Hi-res canvas texture for ALL token data (replaces 2D card)
      const jmboCanvas = document.createElement("canvas");
      jmboCanvas.width = 1024; jmboCanvas.height = 512;
      const jmboCtx = jmboCanvas.getContext("2d");
      const jmboTex = new THREE.CanvasTexture(jmboCanvas);
      jmboTex.minFilter = THREE.LinearFilter;

      // Screen mesh — wide curved screen
      const jmboW = 16, jmboH = 8;
      const jmboGeo = new THREE.PlaneGeometry(jmboW, jmboH, 32, 1);
      // Concave curve — edges closer to viewer for wrap-around feel
      const jmboPositions = jmboGeo.attributes.position;
      for (let i = 0; i < jmboPositions.count; i++) {
        const x = jmboPositions.getX(i);
        jmboPositions.setZ(i, (x * x) * 0.01);
      }
      jmboGeo.computeVertexNormals();

      const jmboMat = new THREE.ShaderMaterial({
        uniforms: { uScreen: { value: jmboTex }, uTime: { value: 0 } },
        vertexShader: `
        ...
        fragmentShader: `
          uniform sampler2D uScreen; uniform float uTime;
          varying vec2 vUv; varying vec3 vN;
          void main(){
            vec4 screen = texture2D(uScreen, vUv);
            // LED pixel grid effect
            vec2 px = fract(vUv * vec2(512.0, 256.0));
            ...
            color *= 0.8 + edge * 0.4;
            // Scanline
            float scan = sin(vUv.y * 512.0 + uTime * 3.0) * 0.03 + 0.97;
            color *= scan;
            gl_FragColor = vec4(color, 1.0);
          }`,
        side: THREE.DoubleSide,
      });
      const jmboScreen = new THREE.Mesh(jmboGeo, jmboMat);
      jmboScreen.position.set(0, 10, -8.5);
      scene.add(jmboScreen);

      // Frame — dark metallic border around screen
      const frameW = jmboW + 0.6, frameH = jmboH + 0.5;
      const frameGeo = new THREE.PlaneGeometry(frameW, frameH);
      const frameMat = new THREE.MeshStandardMaterial({ color: 0x0a0a12, metalness: 0.9, roughness: 0.2, emissive: 0x000000, emissiveIntensity: 0 });
      const jmboFrame = new THREE.Mesh(frameGeo, frameMat);
      jmboFrame.position.set(0, 10, -8.6);
      scene.add(jmboFrame);

      // Glow behind frame — LED bleed
      const glowGeo = new THREE.PlaneGeometry(frameW + 1, frameH + 0.5);
      const glowMat = new THREE.MeshBasicMaterial({ color: 0xff2d78, transparent: true, opacity: 0.0, side: THREE.DoubleSide });
      const jmboGlow = new THREE.Mesh(glowGeo, glowMat);
      jmboGlow.position.set(0, 10, -8.8);
      scene.add(jmboGlow);
      ...
      // ═══ CONCERT LASERS — sweeping beams from behind stage ═══
      const LASER_COUNT = 6;
      ...
      for (let i = 0; i < LASER_COUNT; i++) {
        ...
        beam.position.set((i - LASER_COUNT / 2 + 0.5) * 2.5, 14, -8);
      ...
      threeRef.current = { scene, camera, renderer, spotPink, warm1, warm2, domeMat, floorMat, wallMat,
        ...
        jmboCtx, jmboTex, jmboMat, jmboScreen, jmboFrame, jmboGlow,
        lasers, laserGroup };

      // ═══ DEBUG GUI — tune jumbotron position ═══
      const gui = new GUI({ title: "🎛️ Jumbotron" });
      gui.domElement.style.position = "absolute";
      gui.domElement.style.top = "40px";
      gui.domElement.style.right = "4px";
      gui.domElement.style.zIndex = "100";
      const st2 = stateRef.current;
      const guiObj = { Y: st2.jmboY, Radius: st2.jmboR, Tilt: st2.jmboTiltY, Scale: st2.jmboScale, CamH: st2.cameraHeight, CamD: st2.cameraDist };
      gui.add(guiObj, "Y", 4, 16, 0.1).onChange(v => { stateRef.current.jmboY = v; });
      gui.add(guiObj, "Radius", 4, 14, 0.1).onChange(v => { stateRef.current.jmboR = v; });
      gui.add(guiObj, "Tilt", 2, 14, 0.1).onChange(v => { stateRef.current.jmboTiltY = v; });
      gui.add(guiObj, "Scale", 0.5, 2, 0.05).onChange(v => { stateRef.current.jmboScale = v; });
      gui.add(guiObj, "CamH", 5, 25, 0.5).onChange(v => { stateRef.current.cameraHeight = v; });
      gui.add(guiObj, "CamD", 15, 45, 0.5).onChange(v => { stateRef.current.cameraDist = v; });
      ...
      function animate() {
        ...
        // Spin physics — momentum with friction, auto-rotate fallback
        if (!touchRef.current.active) {
          if (Math.abs(st.spinVelocity) > 0.0001) {
            st.cameraAngle += st.spinVelocity;
            st.spinVelocity *= 0.96; // friction — decays to ~0 in 2-3 seconds
          } else if (st.autoRotate) {
            st.cameraAngle += dt * 0.06;
          } else if (Math.abs(st.spinVelocity) <= 0.0001) {
            // Momentum died, resume auto-rotate after a beat
            st.autoRotate = true;
          }
        }
        ...
        // ═══ JUMBOTRON — MAGICAL BILLBOARD: always faces camera, shows ALL data ═══
        if (threeRef.current.jmboMat) {
          threeRef.current.jmboMat.uniforms.uTime.value = t;
          // Billboard: position opposite camera, always face viewer
          const camAng = st.cameraAngle;
          const jbDirX = -Math.sin(camAng), jbDirZ = -Math.cos(camAng);
          const T = threeRef.current;
          const jR = st.jmboR;
          const jY = st.jmboY;
          const jTilt = st.jmboTiltY;
          const jScale = st.jmboScale;
          T.jmboScreen.position.set(jbDirX * jR, jY, jbDirZ * jR);
          T.jmboScreen.lookAt(camera.position.x, jTilt, camera.position.z);
          T.jmboScreen.scale.setScalar(jScale);
          T.jmboFrame.position.set(jbDirX * (jR + 0.1), jY, jbDirZ * (jR + 0.1));
          T.jmboFrame.lookAt(camera.position.x, jY, camera.position.z);
          T.jmboFrame.scale.setScalar(jScale);
          T.jmboGlow.position.set(jbDirX * (jR + 0.3), jY, jbDirZ * (jR + 0.3));
          T.jmboGlow.scale.setScalar(jScale);
          T.jmboGlow.lookAt(camera.position.x, 7.5, camera.position.z);

          // Update canvas at 5fps — now renders ALL token data
          if (Math.floor(t * 5) !== Math.floor((t - dt) * 5)) {
            ...
            if (si) {
              ...
              // ── Row 1: Ticker + CA + Price + Change ──
              const pad = 60; // inner padding so text never clips screen edges
              jx.font = "bold 44px 'Inter', sans-serif";
              jx.fillStyle = "#fff";
              jx.textAlign = "left";
              jx.fillText(si.coin.ticker, pad, 50);
              // CA
              const tickW = jx.measureText(si.coin.ticker).width;
              jx.font = "500 20px 'JetBrains Mono', monospace";
              jx.fillStyle = "#555";
              jx.fillText(si.coin.ca.slice(0,4) + "..." + si.coin.ca.slice(-4), pad + tickW + 10, 48);
              // Price right-aligned
              jx.font = "bold 38px 'JetBrains Mono', monospace";
              jx.fillStyle = "#fff";
              jx.textAlign = "right";
              jx.fillText("$" + si.coin.price, W - pad, 46);
              // Change
              jx.font = "bold 22px 'JetBrains Mono', monospace";
              jx.fillStyle = priceColor;
              jx.fillText((pos2 ? "▲ " : "▼ ") + si.coin.change, W - pad, 74);
              jx.textAlign = "left";

              // ── Row 2: by caller @ followers · MC · FDV · LIQ · VOL · holders ──
              const row2Y = 98;
              jx.font = "500 18px 'JetBrains Mono', monospace";
              jx.fillStyle = "#ff2d78";
              jx.fillText("by " + si.name, pad, row2Y);
              const nameW = jx.measureText("by " + si.name).width;
              jx.fillStyle = "#666";
              jx.fillText(" @ " + (si.followers || "100K"), pad + nameW, row2Y);
              // Stats
              jx.font = "500 18px 'JetBrains Mono', monospace";
              const holders = si.txStats?.holders ? (si.txStats.holders > 999 ? (si.txStats.holders / 1000).toFixed(1) + "K" : si.txStats.holders) : "";
              const stats = "MC " + si.coin.mcap + " · FDV " + (si.coin.fdv || si.coin.mcap) + " · LIQ " + (si.coin.liq || "—") + " · VOL " + (si.coin.vol || "—") + (holders ? " · " + holders + " hldr" : "");
              jx.fillStyle = "#888";
              jx.textAlign = "right";
              jx.fillText(stats, W - pad, row2Y);
              jx.textAlign = "left";

              // ── Chart area (big) ──
              const cx = pad, cy = 114, cw = W - pad * 2, ch = 240;
              if (cd && cd.length > 1) {
                ...
                jx.moveTo(cx, cy + ch);
                cd.forEach((v, i) => {
                  const x2 = cx + (i / (cd.length - 1)) * cw;
                  const y2 = cy + ch - ((v - minV) / range) * ch;
                  jx.lineTo(x2, y2);
                });
                jx.lineTo(cx + cw, cy + ch);
                jx.closePath();
                const grd = jx.createLinearGradient(0, cy, 0, cy + ch);
                grd.addColorStop(0, glowC + "0.25)");
                grd.addColorStop(1, glowC + "0.0)");
                jx.fillStyle = grd;
                jx.fill();
                // Chart line
                jx.beginPath();
                ...
                jx.strokeStyle = priceColor;
                jx.lineWidth = 4;
                jx.shadowColor = priceColor;
                jx.shadowBlur = 16;
                jx.stroke();
                jx.shadowBlur = 0;
              }
              ...
              const barY = cy + ch + 20;
              if (si.txStats) {
                const { buys, sells, buyVol, sellVol } = si.txStats;
                const buyPct = buys / (buys + sells);
                // Labels
                jx.font = "bold 18px 'JetBrains Mono', monospace";
                jx.fillStyle = "#00ff88";
                jx.textAlign = "left";
                jx.fillText("▲ " + buys + " / $" + buyVol + "K", cx, barY);
                jx.fillStyle = "#ff4444";
                jx.textAlign = "right";
                jx.fillText(sells + " / $" + sellVol + "K ▼", W - cx, barY);
                jx.textAlign = "left";
                // Bar
                const bY = barY + 8, bH = 8;
                jx.fillStyle = "#00ff88";
                jx.beginPath(); jx.roundRect(cx, bY, cw * buyPct - 2, bH, 4); jx.fill();
                jx.fillStyle = "#ff4444";
                jx.beginPath(); jx.roundRect(cx + cw * buyPct + 2, bY, cw * (1 - buyPct) - 2, bH, 4); jx.fill();
              }

              // ── Vote bar ──
              const voteY = barY + 44;
              const totalV = vt.up + vt.down || 1;
              const upPct = vt.up / totalV;
              jx.font = "bold 22px 'JetBrains Mono', monospace";
              jx.fillStyle = "#00ff88";
              jx.textAlign = "left";
              jx.fillText("🚀 " + vt.up, cx, voteY);
              jx.fillStyle = "#ff4444";
              jx.textAlign = "right";
              jx.fillText(vt.down + " 💀", W - cx, voteY);
              jx.textAlign = "left";
              // Vote bar
              const vbY = voteY + 8, vbH = 6;
              jx.fillStyle = "#00ff88";
              jx.beginPath(); jx.roundRect(cx, vbY, cw * upPct - 2, vbH, 3); jx.fill();
              jx.fillStyle = "#ff4444";
              jx.beginPath(); jx.roundRect(cx + cw * upPct + 2, vbY, cw * (1 - upPct) - 2, vbH, 3); jx.fill();

              // ── Footer: caller earnings + countdown timer ──
              const footY = H - 20;
              ...
              if (earned > 0) {
                ...
                jx.fillStyle = "#00ff88";
                jx.textAlign = "left";
                jx.fillText("caller earned $" + earned.toFixed(2), cx, footY);
              }
              const timeLeft = speakerTimeRef.current;
              const timePct = timeLeft / 45;
              jx.font = "500 16px 'JetBrains Mono', monospace";
              jx.fillStyle = timeLeft > 10 ? "#ff2d78" : timeLeft > 5 ? "#ffa500" : "#ff4444";
              jx.textAlign = "right";
              jx.fillText(Math.ceil(timeLeft) + "s", W - pad, footY);
              jx.textAlign = "left";
              // Timer bar at very bottom
              jx.fillStyle = "rgba(255,255,255,0.06)";
              jx.fillRect(0, H - 4, W, 4);
              jx.fillStyle = timeLeft > 10 ? "#ff2d78" : timeLeft > 5 ? "#ffa500" : "#ff4444";
              jx.fillRect(0, H - 4, W * timePct, 4);

            } else {
              // No active call — epic PODIUM logo + stats
              jx.font = "bold 72px 'Inter', sans-serif";
              jx.fillStyle = "#ff2d78";
              jx.textAlign = "center";
              jx.shadowColor = "#ff2d78";
              jx.shadowBlur = 30;
              jx.fillText("trench.fm", W / 2, 140);
              jx.shadowBlur = 0;
              jx.font = "400 22px 'Inter', sans-serif";
              jx.fillStyle = "#555";
              jx.fillText("step up to the mic — call your coin", W / 2, 180);
              // Platform stats
              jx.font = "500 18px 'JetBrains Mono', monospace";
              jx.fillStyle = "#333";
              jx.fillText("──────────────────────────────", W / 2, 220);
              // Show top callers if we have earnings data
              ...
              if (topCallers.length > 0) {
                ...
                jx.fillStyle = "#555";
                jx.fillText("TOP CALLERS", W / 2, 258);
                topCallers.forEach(([name, amt], i) => {
                  const y2 = 290 + i * 30;
                  const medal = i === 0 ? "🥇" : i === 1 ? "🥈" : "🥉";
                  jx.font = "500 18px 'Inter', sans-serif";
                  jx.fillStyle = "#aaa";
                  jx.fillText(medal + " " + name + "  ·  $" + amt.toFixed(2) + " earned", W / 2, y2);
                });
              }
              // Live viewer count
              jx.font = "500 16px 'JetBrains Mono', monospace";
              jx.fillStyle = "#00ff88";
              jx.fillText("● " + (400 + Math.floor(Math.random() * 50)) + " watching", W / 2, H - 30);
              jx.textAlign = "left";
            }
            T.jmboTex.needsUpdate = true;
          }
          // Glow pulses + buy flash
          if (T.jmboGlow) {
            const flashAge = t - buyFlashRef.current;
            if (flashAge < 1.5) {
              // Green flash burst on buy
              const flash = Math.max(0, 1 - flashAge / 1.5);
              T.jmboGlow.material.color.setHex(0x00ff88);
              T.jmboGlow.material.opacity = 0.06 + flash * 0.35;
            } else {
              T.jmboGlow.material.color.setHex(0xff2d78);
              T.jmboGlow.material.opacity = 0.04 + Math.sin(t * 2) * 0.02;
            }
          }
        }

        // ═══ CONCERT LASERS — sweep beams over the crowd ═══
        if (threeRef.current.lasers) {
          // Lasers orbit with the jumbotron (magical — always behind stage from camera)
          const camAng2 = st.cameraAngle;
          threeRef.current.laserGroup.position.set(-Math.sin(camAng2) * 8, 0, -Math.cos(camAng2) * 8);
          threeRef.current.laserGroup.lookAt(camera.position.x, 0, camera.position.z);
        ...
        for (let i = 0; i < count; i++) {
          ...
          // ── Position & scale — speaker is centered on stage at 1.8x
          let cx, cz, sv;
          if (isOnStage) {
            cx = 0; cz = 0;
            if (st.teleportPhase === "beam_in") {
              const s = Math.min((st.teleportTimer - 0.25) / 0.3, 1);
              sv = 0.001 + s * 1.799; // scale from tiny → 1.8
            } else {
              sv = 1.8;
            }
          } else {
          ...
          sc.set(sv, sv, sv);
          pos.set(cx + headTilt * 0.12 * pScale, (1.32 + headBob + jumpH) * pScale + stageY, cz);
  ...
  const handleStepUp = useCallback((ca, thesis) => {
    ...
    setCardExpanded(false);
    setChartData(generateChart(matchedCoin.positive));
    setVotes({ up: Math.floor(Math.random() * 20) + 5, down: Math.floor(Math.random() * 5) });
    setUserVoted(null);
    setShowStepUpModal(false);
    setStepUpCA("");
    setStepUpThesis("");
    triggerAnnouncement({ name: c.name, coin: matchedCoin, thesis, followers });
  ...
  const onDragMove = useCallback((x, y, e) => {
    ...
    stateRef.current.cameraHeight = Math.max(5, Math.min(25, tr.sh + (y - tr.startY) * 0.04));
  }, []);
  const onDragEnd = useCallback(() => {
    const tr = touchRef.current;
    tr.active = false;
    // Launch spin with flick velocity
    stateRef.current.spinVelocity = -tr.velX * 0.004;
  ...
  return (
    <div style={{ width: "100%", height: "100vh", background: "#0a0008", position: "relative", overflow: "hidden", fontFamily: "'Segoe UI',system-ui,sans-serif", maxWidth: 430, margin: "0 auto" }}>
      ...
      {/* ═══ ANNOUNCEMENT OVERLAY — "ALL I NEED IS ONE MIC" ═══ */}
      {announcement && (
        <div style={{ position: "absolute", inset: 0, zIndex: 200, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", pointerEvents: "none", animation: "announceOverlay 3.2s ease forwards" }}>
          {/* Dark backdrop */}
          <div style={{ position: "absolute", inset: 0, background: "radial-gradient(ellipse at center, rgba(10,0,8,0.85) 0%, rgba(10,0,8,0.95) 100%)", backdropFilter: "blur(8px)" }} />
          {/* Glow burst behind name */}
          <div style={{ position: "absolute", width: 300, height: 300, borderRadius: "50%", background: "radial-gradient(circle, rgba(255,45,120,0.3) 0%, rgba(255,102,34,0.1) 40%, transparent 70%)", animation: "announceGlow 3.2s ease forwards" }} />
          {/* Content */}
          <div style={{ position: "relative", textAlign: "center", padding: "0 30px" }}>
            {/* Top line accent */}
            <div style={{ width: 40, height: 2, background: "linear-gradient(90deg,transparent,#ff2d78,transparent)", margin: "0 auto 16px", animation: "announceLine 3.2s ease forwards" }} />
            {/* "TAKES THE MIC" label */}
            <div style={{ fontFamily: "'Inter'", fontWeight: 700, fontSize: 10, color: "#ff2d78", letterSpacing: 4, textTransform: "uppercase", marginBottom: 8, animation: "announceLabel 3.2s ease forwards" }}>
              Takes the Mic
            </div>
            {/* CALLER NAME — the hero */}
            <div style={{ fontFamily: "'Press Start 2P'", fontSize: 22, color: "#fff", textShadow: "0 0 40px rgba(255,45,120,0.6), 0 0 80px rgba(255,45,120,0.3), 0 2px 20px rgba(0,0,0,0.8)", letterSpacing: 1, lineHeight: 1.4, animation: "announceName 3.2s ease forwards", marginBottom: 14 }}>
              {announcement.name.toUpperCase()}
            </div>
            {/* Coin + mcap line */}
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, animation: "announceSub 3.2s ease forwards", marginBottom: 10 }}>
              <span style={{ fontFamily: "'JetBrains Mono'", fontSize: 16, fontWeight: 700, color: "#fff", letterSpacing: -0.5 }}>{announcement.coin.ticker}</span>
              <span style={{ width: 4, height: 4, borderRadius: "50%", background: "#333" }} />
              <span style={{ fontFamily: "'JetBrains Mono'", fontSize: 12, fontWeight: 500, color: "#666", letterSpacing: -0.3 }}>{announcement.coin.mcap}</span>
              <span style={{ fontFamily: "'JetBrains Mono'", fontSize: 12, fontWeight: 600, color: announcement.coin.positive ? "#00ff88" : "#ff4444", letterSpacing: -0.3 }}>{announcement.coin.change}</span>
            </div>
            {/* Thesis preview */}
            {announcement.thesis && (
              <div style={{ fontFamily: "'Inter'", fontSize: 12, fontWeight: 400, color: "#777", fontStyle: "italic", maxWidth: 280, margin: "0 auto", lineHeight: 1.5, animation: "announceSub 3.2s ease forwards", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
                "{announcement.thesis}"
              </div>
            )}
            {/* Bottom line accent */}
            <div style={{ width: 40, height: 2, background: "linear-gradient(90deg,transparent,#ff2d78,transparent)", margin: "16px auto 0", animation: "announceLine 3.2s ease forwards" }} />
            {/* Followers pill */}
            <div style={{ marginTop: 12, animation: "announceSub 3.2s ease forwards" }}>
              <span style={{ fontFamily: "'JetBrains Mono'", fontSize: 9, fontWeight: 500, color: "#555", letterSpacing: -0.3, padding: "3px 10px", borderRadius: 20, border: "1px solid rgba(255,255,255,0.06)", background: "rgba(255,255,255,0.03)" }}>
                {announcement.followers || "100K"} followers
              </span>
            </div>
          </div>
        </div>
